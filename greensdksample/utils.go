...
func addRequestHeader(requestBody string, req *http.Request, clientInfo string, path string, accessKeyId string, accessKeySecret string) {
	...
	req.Header.Set("Authorization", "acs"+" "+accessKeyId+":"+singature(acsHeaderKeyArray, acsHeaderValueArray, base64Md5Str, clientInfo, path, accessKeySecret))
}

func singature(acsHeaderKeyArray []string, acsHeaderValueArray []string, md5Str string, clientInfo string, path string, accessKeySecret string) string {
	b := bytes.Buffer{}
